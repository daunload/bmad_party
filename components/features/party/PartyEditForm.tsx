...
import { updatePartyAction } from '@/app/actions/party';
...
export function PartyEditForm({ party }: PartyEditFormProps) {
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(party.imageUrl || null);

  // Format date for input (YYYY-MM-DD)
  const formattedDate = party.date instanceof Date 
    ...
    <form action={handleSubmit} className="flex flex-col space-y-4 bg-gray-50 px-4 py-8 sm:px-16">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          ...
          <input
            ...
            min={2}
            max={100}
            defaultValue={party.maxParticipants}
            className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500"
          />
      ...
      <div>
        ...
        <select
          ...
          defaultValue={party.minPopularityRating || ''}
          className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500"
        >
      ...
      <div>
        ...
        />
        <input type="hidden" id="imageUrl" name="imageUrl" defaultValue={party.imageUrl || ''} />
